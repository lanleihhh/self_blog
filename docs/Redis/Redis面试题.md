...
Redis中的SETNX命令可以实现锁的排他性，当key存在的时候返回0，不存在的时候返回1

使用expire命令 设置锁的失效时间，避免死锁问题

业务逻辑没有执行完，锁就过期了，这时写一个定时任务，对指定的Key进行续期



Redisson开源组件提供了分布式锁的封装实现，内置了一个Watch Dog(看门狗)的机制对Key做续期



如果redis搭建了高可用集群，出现了主从切换导致key失效的问题，也可能导致多个线程或进程抢占同一个锁资源的情况，提供了RedLock的解决办法
